...
int main(int argc, char **argv) {
	...
	// int shutter_us = 10 * 1000;
	// float analog_gain = 1.0;
	// control_list.set(libcamera::controls::EXPOSURE_TIME, shutter_us);
	// control_list.set(libcamera::controls::ANALOGUE_GAIN, analog_gain);
	// control_list.set(libcamera::controls::AE_ENABLE, false);
	// if (!controls_.contains(controls::AeExposureMode))
	// 	controls_.set(controls::AeExposureMode, options_->exposure_index);
	...
	int i = 0;
	int exposures[] = {1000, 10000, 100000};
	...
	while (1) {
		...
		libcamera::ControlList control_list(camera->controls());
		control_list.set(libcamera::controls::EXPOSURE_TIME, exposures[i%3]);
		control_list.set(libcamera::controls::ANALOGUE_GAIN, gains[i%3]);
		control_list.set(libcamera::controls::AE_ENABLE, false);
		i++;
		if (camera->start(&control_list)) 
			throw std::runtime_error("failed to start camera");
		...
		//request->controls().set(libcamera::controls::EXPOSURE_TIME, exposures[i % 3]);
		//request->controls().set(libcamera::controls::ANALOGUE_GAIN, gains[i % 3]);
		//request->controls().set(libcamera::controls::AE_ENABLE, false);
